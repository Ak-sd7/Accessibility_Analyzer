	...
	async examine(page: Page): Promise<Issue[]>{
		return await page.evaluate(()=>{
			...
			images.forEach((image: HTMLImageElement, ind: number)=>{
				...
				const spareString = ["image of", "picture of", "photo of", "graphic of", "video of"];
				let isDecorativeImg: boolean = false;

				if(role === "presentation" || role === "none")
					isDecorativeImg = true;
				
				if(isDecorativeImg && altValue === "")
					return;
				...
				if(!hasAltAtrr && !ariaLabel && !ariaLabelledby) {
					issues.push({
						...
						impact: 'Assistive screen readers cannot describe this image to users',
				...
				if(hasAltAtrr && spareString.some(phrase => altValue.toLowerCase().includes(phrase))) {
					issues.push({
						element: image.outerHTML,
						selector: `img:nth-child(${ind + 1})`,
